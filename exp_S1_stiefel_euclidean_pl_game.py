"""
=============================================================================
SYNTHETIC EXPERIMENT S1 — Stiefel x Euclidean bilevel PL game
=============================================================================

Purpose
-------
The cleanest proof-of-concept that the classical Hessian-inverse
hypergradient (Han et al., NeurIPS 2024) BREAKS when the lower level is only
PL, and that R-HJFBiO (ours) continues to work.  Two parallel runs on the
SAME problem class, one in the strongly-convex (SC) regime where both
methods succeed, one in the PL-not-SC regime where HINV fails.

Problem setup
-------------
    min_{W in St(d, r)}  f(W, y*(W))
    s.t.                 y*(W) in argmin_y  g(W, y)

with f = ||y - c||^2 + 0.1 * tr(W^T X Y) on R^p,
and  g = 0.5 * ||A y - b(W)||^2 + (mu_reg/2) * ||y||^2 .

  (A) SC:   mu_reg = 0.1,  A square and well-conditioned.
            Hess_y g = A^T A + mu_reg I  is uniformly positive definite.
            Classical implicit-differentiation theory applies; HINV works.

  (B) PL:   mu_reg = 0,    A is fat (m = p/3 rows).
            Hess_y g = A^T A  has a ~(2p/3)-dimensional null space.
            Strongly convex analysis breaks; PL analysis still applies.
            HINV tries to invert a rank-deficient Hessian and fails.

Baselines compared
------------------
  RHGD-HINV (Han et al.)  : direct Hessian inverse.  FAILS under (B).
  R-HJFBiO (Ours)         : spectral clipping + finite differences.
                            Works in both regimes.

Concerns (Andi's review) addressed
----------------------------------
  [Direct]  Demonstrates the MAIN claim of the paper:  Hessian-inverse
            baselines fail in PL-not-SC while R-HJFBiO does not.

Framework
---------
  PyTorch + geoopt.  (Requires: torch, geoopt, numpy, matplotlib.)

How to run
----------
    python exp_S1_stiefel_euclidean_pl_game.py

Output
------
  outputs/synthetic_S1_stiefel_euclidean.png   -- 2x2 grid.  Top row: SC
                                                  regime.  Bottom row: PL
                                                  regime, Han et al. fails
                                                  with a "Hessian singular"
                                                  annotation.
  Console                                       -- Eigenvalue ranges, final
                                                  F and ||grad F||.

Provenance
----------
  Original name:  synthetic.py  (preserved verbatim below, only headers
  rewritten to match the rest of the experiment suite).
=============================================================================
"""
...
class BilevelProblem:
    ...
    def __init__(self, d, r, p, setting="sc"):
        ...
        if setting == "sc":
            # g(W, y) = 0.5 * ||A y - b(W)||^2 + (mu/2) * ||y||^2
            # Hess_y g = A^T A + mu I  (positive definite, invertible)
            self.mu_reg = 0.1
            A0 = torch.randn(p, p)
            ...
            # g(W, y) = 0.5 * ||A y - b(W)||^2,  A fat  (m < p).
            # Hess_y g = A^T A  has nontrivial null space  => NOT s.c.
    ...
    def hypergradient_hjfbio(self, W, y, v_aux, mu_clip=0.1, delta_eps=1e-4):
        ...
        # Jacobian-free cross term via finite difference.
...
def rhgd_han(prob, K=300, S=15, eta_x=0.005, eta_y=0.03):
    """RHGD-HINV (Han et al., NeurIPS 2024)."""
    mf = prob.stiefel
    ...
    for k in range(K):
        # Inner loop: lower-level gradient descent.
        yk = y.clone()
        for _ in range(S):
...
def rhjfbio_ours(prob, T=300, S=15, gamma=0.005, lam=0.03, tau=0.01,
                 mu_clip=0.1):
    """R-HJFBiO (ours), Algorithm 1 of the paper."""
    mf = prob.stiefel
    W, _ = torch.linalg.qr(torch.randn(prob.d, prob.r))
    y = torch.randn(prob.p) * 0.1
...
def main():
    ...
    print(f"  H_y g eigenvalues: [{eigs.min():.3f}, {eigs.max():.3f}], "
          f"kappa={eigs.max() / eigs.min():.1f}")
    ...
    eigs = torch.linalg.eigvalsh(H)
    print(f"  H_y g eigenvalues (first 6): "
          f"{[f'{e:.4f}' for e in eigs[:6].tolist()]}")
    print(f"  Zero eigenvalues: {(eigs < 1e-8).sum().item()}, "
          f"PL mu={prob_pl.pl_mu:.4f}")
    ...
    def plot_series(ax, han, ours, ylabel, title):
        ...
        if has_nan:
            ...
            ax.semilogy(han, label="RHGD-HINV (Han et al.)",
                        color=colors["han"], lw=2)

        ax.semilogy(ours, label="R-HJFBiO (Ours)",
                    color=colors["ours"], lw=2)
    ...
    plot_series(axes[0, 1], h_sc_han["grad_norm"], h_sc_ours["grad_norm"],
                "||grad F||", "(A) Strongly Convex g - Gradient Norm")
    ...
    plot_series(axes[1, 1], h_pl_han["grad_norm"], h_pl_ours["grad_norm"],
                "||grad F||", "(B) PL (Not S.C.) g - Gradient Norm")

    fig.suptitle("Bilevel Optimization on St(d, r): RHGD-HINV vs R-HJFBiO",
                 fontsize=14, fontweight="bold", y=0.99)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig("outputs/synthetic_S1_stiefel_euclidean.png",
    ...
    print("=" * 55)
    print(f"\n(A) Strongly Convex:")
    print(f"    Han  final F={h_sc_han['F'][-1]:.4f}  "
          f"|grad F|={h_sc_han['grad_norm'][-1]:.4f}")
    print(f"    Ours final F={h_sc_ours['F'][-1]:.4f}  "
          f"|grad F|={h_sc_ours['grad_norm'][-1]:.4f}")
    ...
    if nan_han:
        ...
        print(f"    Han  final F={h_pl_han['F'][-1]:.4f}  "
              f"|grad F|={h_pl_han['grad_norm'][-1]:.4f}")
    print(f"    Ours final F={h_pl_ours['F'][-1]:.4f}  "
          f"|grad F|={h_pl_ours['grad_norm'][-1]:.4f}")
    print("\nDone!")
